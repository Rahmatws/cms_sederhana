...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_comment']) && isset($_SESSION['user_id'])) {
    ...
    if (strlen($comment) < 2) {
        $comment_flash = '<div class="alert alert-danger">Komentar terlalu singkat.</div>';
    } else {
        $stmt = $pdo->prepare("INSERT INTO comments (post_id, user_id, content) VALUES (?, ?, ?)");
        $stmt->execute([$id, $_SESSION['user_id'], $comment]);
        $comment_flash = '<div class="alert alert-success">Komentar berhasil ditambahkan!</div>';
    }
}
// Ambil komentar
$stmt = $pdo->prepare("SELECT comments.*, users.username FROM comments LEFT JOIN users ON comments.user_id = users.id WHERE comments.post_id = ? ORDER BY comments.created_at DESC");
$stmt->execute([$id]);
...
<div class="wrapper">
    <nav class="main-header navbar navbar-expand-md navbar-light navbar-white">
        <div class="container">
            <a href="posts.php" class="navbar-brand"><b>MyCMS</b> Rahmat</a>
        </div>
    </nav>
            ...
            <div class="container">
                ...
                <div class="card mx-auto mt-4" style="max-width:700px;">
                    <div class="card-body">
                        ...
                        <?php if ($comment_flash) echo $comment_flash; ?>
                        ...
                        <?php else: ?>
                        <div class="alert alert-info">Login untuk menulis komentar.</div>
                        <?php endif; ?>
                ...
                <div class="text-center mt-4">
                    <a href="posts.php" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Kembali ke Daftar Post</a>
                </div>
            </div>
        </div>
    </div>
